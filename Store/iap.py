"""Curve Pro: the one in-app purchase, and the switch from paid to free.
...
PRODUCT_ID = "com.mattbusel.curve.pro"
NAME = "Curve Pro"
PRICE = "2.99"
DESCRIPTION = "Every term, what-if GPA, drop lowest, PDF report."  # 55 chars max
REVIEW_NOTE = (
    "Non-consumable, one-time unlock. The app is free; the current term, courses, scores, the weighted grade, "
    "the need dial and the term GPA are free with no limit. Pro unlocks more than one term with a cumulative GPA, "
    "the what-if projection, drop-lowest in categories, and the PDF report and CSV export. To see the paywall: "
    "open the GPA tab and tap any letter in the What if card, or open the Report tab and tap Share PDF and CSV "
    "with Curve Pro. Restore purchase is on the paywall and on the Curve Pro card at the bottom of the GPA tab."
)
